...
------ Training Function
------------------------------------------------------
-- retrieve a view (same memory) of the parameters and gradients of
-- these (wrt loss) of the model (Global)
...
function train_one_epoch(dataset)
    ...
    for mini_batch_start = 1, dataset:size(), batch_size do
        ...
        -- and df/dx is the gradient of the loss wrt to the parameters
        ...
        config = {learningRate = params.learning_rate, mementum = params.momentum }
        ...
        xlua.progress(mini_batch_start, dataset:size())
    end
